...
if ( ! class_exists( 'TR_Admin_Page_Class_V10') ) :
    ...
    class TR_Admin_Page_Class_V10 {
        ...
        public function __construct($args) {
            if(!is_array(@$args['fields']))$args['fields'] = array();
            if(isset($args['path']))
            {
                $this->folder_root = $args['path'];
            }else
            {
            ...
            {
                $tab = isset($_REQUEST['pagetab'])? $_REQUEST['pagetab'] : @array_shift(array_keys($args['tabs']));
                if($tab!='this')
                {
                    $current_tab = $args['tabs'][$tab];
                    if(is_array($current_tab) && !empty($current_tab['option']))
                    {
                        $this->option_group = $current_tab['option'];
                        $args['option_group'] = $current_tab['option'];
                    }

                }

                foreach($args['tabs'] as $tslug => $ttext)
                {
                    add_theme_support('display_tab_'.$tslug);
                }
            ...
            add_action('admin_init',array($this, 'admin_redirect_download_files'));
            ...
            $menu_action = @$args['network'] && WP_NETWORK_ADMIN===true? 'network_admin_menu': 'admin_menu';
            ...
            if(!is_array($args['menu'])) {
                ...
                add_action($menu_action, array($this, 'AddMenuSubPage'),999);
            }else{
                //top page
                $this->Top_Slug = $args['menu']['top'];
                add_action($menu_action, array($this, 'AddMenuTopPage'),999);
            }
            ...
            if (isset($args['use_with_theme'])){
                if ($args['use_with_theme'] === true){
                    ...
                    $this->SelfPath = plugins_url( 'admin',  $this->folder_root .'/plug.php' );
                }else{
            ...
            //import export
            add_action( 'wp_ajax_apc_import_tr', array( $this, 'import' ) );
            add_action( 'wp_ajax_apc_export_tr', array( $this, 'export' ) );
        ...
        public function AddMenuTopPage() {
            ...
            $pageslug = $this->createSlug();
            $page = add_menu_page($this->args['menu_title'], $this->args['menu_title'], $this->args['capability'], $pageslug, array($this, 'Menu_Callback'), $this->args['icon_url'], $this->args['position']);
            ...
            if ($page){
                ...
                //check child menu
                if(is_array($this->args['tabs']))
                {
                    foreach($this->args['tabs'] as $tslug => $ttext)
                    {
                        if(WP_NETWORK_ADMIN===true && @$ttext['network']===false)
                        {
                            continue;
                        }
                        if(is_array($ttext) && @$ttext['menu'])
                        {
                            if($ttext['menu']===true)$ttext['menu'] = $ttext['label'];
                            if($tslug=='this')
                            {
                                remove_submenu_page($pageslug,$pageslug);
                                $page = add_submenu_page($pageslug, $ttext['label'], $ttext['menu'], $this->args['capability'], 'admin.php?page='.$pageslug.'&pagetab='.$tslug);
                                continue;
                            }
        ...
        public function AddMenuSubPage() {
            ...
            $page = add_submenu_page($this->Top_Slug, $this->args['menu_title'], $this->args['menu_title'], $this->args['capability'], $this->createSlug(), array($this, 'Menu_Callback'));
            if ($page){
                $this->_Slug = $page;
                add_action('load-'.$page, array($this,'Load_page_hooker'));
            }
        ...
        public function Load_page_hooker(){
            ...
            if(is_array($this->args['tabs']))
            {
                $tab = isset($_REQUEST['pagetab'])? $_REQUEST['pagetab'] : array_shift(array_keys($this->args['tabs']));
                do_action('admin_head_tab_'.$tab);
                if($tab!='this')
                {
                    $current_tab = $this->args['tabs'][$tab];
                    if(is_array($current_tab) && !empty($current_tab['option']))
                    {
                        $this->loadconfig($tab);
                        $loaded_config = true;
            ...
            //pluploader code
            add_action('admin_head-'.$page, array($this,'plupload_head_js'),1);
            //scripts and styles
            ...
            if(isset($_POST['action']) && $_POST['action'] == 'save') {
                $is_restore = isset($_POST['restore']) && $_POST['restore'] == 'default';
                if($is_restore)
                {
                    $this->restore();
                }else
        ...
        public function plug_dequeue_js()
        {
            wp_dequeue_script('jquery.tools');
            wp_dequeue_script('jquery.libs');
        }
        ...
        /**
         * Creates an unique slug out of the menu_title and the current menu_slug
         *
         * @since 0.1
         * @access private
         */
     ...
     */
        ...
        public function loadconfig($id = '')
        {
            ...
            if(empty($id))
            {
                $id = $this->args['option_group'];
                add_action('display_tab_this',array($this,'Displaypage'));
            }else
            {
                add_action('display_tab_'.$id,array($this,'Displaypage'));
            }
            $fileconfig = $this->folder_root.'/inc/'.$id.'-options.php';

            if(is_file($fileconfig))
                include($fileconfig);

            if($this->args['usebackup'] && isset($this->_fields[1]['links']))
            {
                $this->_fields[1]['links'] = array_merge($this->_fields[1]['links'],array('backup_config_tab'=>'Backup & Restore'));

                $this->OpenTab('backup_config_tab');
                $this->Title('Backup & Restore Settings');
        ...
        static public function showtabs($tabs,$slug='pagetab')
        {
            $tab = isset($_REQUEST[$slug])? $_REQUEST[$slug] : array_shift(array_keys($tabs));
            ?>
            <div class="top_tabs">
                <ul>
                    <?php
                    $ptype = isset($_REQUEST['post_type'])? 'post_type='.$_REQUEST['post_type'].'&':'';
                    foreach($tabs as $tabslug => $tabtext):
                        if(WP_NETWORK_ADMIN===true && @$tabtext['network']===false)
                        {
                            continue;
                        }
                        if(is_array($tabtext))
                        {
                            $file = $tabtext['file'];
                            $tabtext = $tabtext['label'];
                        }
                        $active_tab = ($tabslug == $tab) ? true : false;

                        ?>
                        <li class="<?php if($active_tab)echo 'active_tab'?>">
                            <a href="?<?php echo $ptype?>page=<?php echo $_GET['page']?>&<?php echo $slug?>=<?php echo $tabslug?>">
                                <?php echo $tabtext?></a>
                        </li>
                    <?php endforeach;?>
                </ul>
            ...
            <?php
            do_action('display_tab_'.$tab);
            if(!empty($tabs[$tab]['file']) && is_file($tabs[$tab]['file']))
            {
        ...
        public function DisplayTabs()
        {
            ?>
            <div class="wrap">
                <?php self::showtabs($this->args['tabs'],'pagetab');?>
            </div>
            <?php
        }
        ...
        public function DisplayPage() {
            ...
            echo '<div class="wrap tradmin">';
            echo '<form method="post" action="" id="admin_options_form" enctype="multipart/form-data">
          <div class="msg_alert_success"></div>
          <div class="header_wrap">
          <div style="float:left">';
            echo apply_filters('admin_page_class_before_title','');
            echo '<h1>'.apply_filters('admin_page_class_h2',$this->args['page_title']).'</h1>'.((isset($this->args['page_header_text']))? $this->args['page_header_text'] : '').' 
          </div>';
            if($this->args['only_support']!==true):

                $buttons .= '<input type="submit" style="margin-left: 20px;" value="Save changes" name="Submit" class="'.apply_filters('admin_page_class_submit_class', 'btn-info').' btn">';

                echo '<div style="float:right;margin:32px 0 0 0">
            '.apply_filters('admin_save_buttons_area',$buttons).'            
          </div>';
            endif;
            echo '<br style="clear:both"><br>
        </div>';
            wp_nonce_field( basename(__FILE__), 'TR_Admin_Page_Class_nonce' );

            if ($this->saved_flag)
                echo '<div class="alert alert-success"><p><strong>'.__('Settings saved.').'</strong></p></div>';

            do_action('admin_page_class_after_title');
            ...
            foreach($this->_fields as $field) {
                ...
                if (method_exists($this,'show_field_' . $field['type'])){
                    if ($this->_div_or_row){echo '<td>';}else{echo apply_filters('admin_page_class_field_container_open','<div class="field" id="field_'.$field['id'].'">',$field);}
                        ...
                        case 'TABS_Listing':
                            ...
                            foreach($field['links'] as $id => $name){
                                ...
                                if(is_array($name))
                                {
                                    ...
                                    foreach($childs as $cid => $cname)
                                    {
                                        $cid = $id.'_'.$cid;
                                        $extra_classes = strtolower(str_replace(' ','-',$cname)).' '.strtolower(str_replace(' ','-',$cid));
                                        $child.= '<li class="'.apply_filters('APC_tab_li_extra_class',$extra_classes).'"><a class="nav_tab_link" href="#'.$cid.'">'.$cname.'</a></li>';
                                    }
                                    $child.='</ul>';
                                }
                                $extra_classes = strtolower(str_replace(' ','-',$name)).' '.strtolower(str_replace(' ','-',$id));
                                echo '<li class="'.apply_filters('APC_tab_li_extra_class',$extra_classes).'"><a class="nav_tab_link" href="#'.$id.'">'.$name.'</a>'.$child.'</li>';
                            }
                            echo '</ul></div><div class="sections_tabs">';
                            break;
                        case 'OpenTab':
                            $this->tab_div = true;
                            ...
                            break;
                        case 'title':
                            echo '<div class="tab_title">'.$field['label'].'</div>';
                            break;
                        case 'subtitle':
          ...
          <input type="button" style="margin-left: 20px;" value="Restore Default" class="btn-warning btn restore_button">
        ...
        public function load_scripts_styles() {
            ...
            wp_enqueue_style( 'Admin_Page_Class', $plugin_path . '/css/admin.css',array(),'1.1.9' );
            wp_enqueue_style('iphone_checkbox',$plugin_path. '/js/iphone-style-checkboxes/style.css');

            // Enqueue admin page Scripts
            wp_enqueue_script( 'Admin_Page_Class', $plugin_path . '/js/Admin_Page_Class.js', array( 'jquery' ), '1.1.9', true );
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
        ...
        public function show_field_text( $field, $meta) {
            ...
            echo "<input type='text' class='at-text' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='{$size}' />";
            $this->show_field_end( $field, $meta );
        }

        public function show_field_texts( $field, $meta) {
            $this->show_field_begin( $field, $meta );
            $size = isset($field['size'])? $field['size'] : 30;
            if(!is_array($meta))$meta= array('');
            ?>
            <div class="field_texts">
                <ol class="listrows"  data-name="<?php echo $field['id']?>" data-size="<?php echo $size?>">
                    <?php
                    foreach($meta as $value):
                        ?>
                        <li><input type='text' class='at-text' name='<?php echo $field['id']?>[]' id='<?php echo $field['id']?>'
                                   value='<?php echo $value?>' size='<?php echo $size?>' /><a></a></li>
                        <?php
                    endforeach;
                    ?>
                </ol>
                <a href="#" class="add_row button">Add Row</a>
            </div>
        ...
        public function show_field_textboxs( $field, $meta) {
            $this->show_field_begin( $field, $meta );
            $size = isset($field['size'])? $field['size'] : 30;
            if(!is_array($meta))$meta= array(array());
            ?>
            <div class="field_texts">
                <ul class="field_titles">
                    <?php foreach($field['fields'] as $i => $fname): ?>
                ...
                <ol class="listrows"  data-name="<?php echo $field['id']?>" data-size="<?php echo $size?>">
                    ...
                    foreach($meta as $value):
                        ?>
                        <li><?php
                        foreach($field['fields'] as $i => $fname):
                            ?>
                            <input type='text' class='at-text' name='<?php echo $field['id']?>[<?php echo $j?>][<?php echo $i?>]' id='<?php echo $field['id']?>'
                                   value='<?php echo @$value[$i]?>' size='<?php echo $size?>' />
                            <?php
                        endforeach;?>
                        <a></a></li><?php
                        $j++;
                ...
                <a href="#" class="add_rowbox button" data-count="<?php echo $j?>" data-keys="<?php echo implode('||',array_keys($field['fields']))?>" data-fields="<?php echo implode('||',$field['fields'])?>">Add Row</a>
            </div>
        ...
        public function show_field_plupload( $field, $meta) {
            ...
            $custom_js_function = '';
            if(!empty($field['js_func']))
            {
                $custom_js_function=' data-js_func="'.$field['js_func'].'" ';
            }
            $html = '
        <input type="hidden" name="'.$id.'" id="'. $id.'" value="'.$meta .'" />
        <div class="plupload-upload-uic hide-if-no-js '.$m1.'" id="'.$id.'plupload-upload-ui" '.$custom_js_function.'>
        ...
        public function show_field_textarea( $field, $meta ) {
            ...
            $meta = stripslashes($meta);
            $readonly = '';
            if($field['static_data'])
            {
                $meta = $field['static_data'];
                $readonly = 'readonly="true"';
            }
            echo "<textarea class='at-textarea large-text' {$readonly} name='{$field['id']}' id='{$field['id']}' cols='60' rows='10'>{$meta}</textarea>";
            $this->show_field_end( $field, $meta );
        }
        ...
        public function show_field_select( $field, $meta ) {
            ...
            if(isset($field['custom'])){
                ...
                echo call_user_func($field['func'],$meta);
            }
            else{
        ...
        public function show_field_radio( $field, $meta ) {
            ...
            $this->show_field_begin( $field, $meta );
            foreach ( $field['options'] as $key => $value ) {
                echo "<label class='forradio'><input type='radio' class='at-radio' name='{$field['id']}' value='{$key}'" . checked( in_array( $key, $meta ), true, false ) . " /> <span class='at-radio-label'>{$value}</span></label>";
            }
            $this->show_field_end( $field, $meta );
        }
        ...
        public function show_field_checkbox( $field, $meta ) {
            ...
            $this->show_field_begin($field, $meta);
            echo "<input type='checkbox' class='rw-checkbox' name='{$field['id']}' id='{$field['id']}'" . checked($meta, true, false) . " target='{$field['target']}' />";
            $this->show_field_end( $field, $meta );
        }
        ...
        public function show_field_image( $field, $meta ) {
            ...
            $height = (isset($field['preview_height']))? $field['preview_height'] : '100px';
            $width = (isset($field['preview_width']))? $field['preview_width'] : '100px';
            if (is_array($meta)){
            ...
            if (is_array($meta) && isset($meta['src']) && $meta['src'] != ''){
                ...
                $html .= "<input class='at-delete_image_button button' type='button' rel='".$field['id']."' value='Delete Image' />";
            }else{
        ...
        public function show_field_color( $field, $meta ) {
            ...
            $this->show_field_begin( $field, $meta );
            echo "<input class='at-color' type='text' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='8' style='background-color:{$meta}' />";
        ...
        public function show_field_custom($field,$meta)
        {
            $options_panel = $this;
            $this->show_field_begin( $field, $meta );
            include_once ($this->folder_root.'/inc/tab-custom-'.$field['id'].'.php');
            $this->show_field_end( $field, $meta );
        }
        /**
         * Show time field.
        ...
        public function restore()
        {
            ...
            update_option($this->option_group,$this->_saved);
            do_action('admin_saved_option_'.$this->option_group,$this);
        }
        ...
        public function save($repeater = false) {
            ...
            //check nonce
            if ( ! check_admin_referer( basename( __FILE__ ), 'TR_Admin_Page_Class_nonce') )
                return;
            ...
            foreach ( $this->_fields as $field ) {
                if(!in_array($field['type'],$skip)){
                    ...
                    // Validate meta value
                    if ( class_exists( 'TR_Admin_Page_Class_Validate' ) && method_exists( 'TR_Admin_Page_Class_Validate', $field['validate_func'] ) ) {
                        $new = call_user_func( array( 'TR_Admin_Page_Class_Validate', $field['validate_func'] ), $new );
                    }
   ...
   */
        ...
        public function addTexts($id,$args,$repeater=false){
            $new_field = array('type' => 'texts','id'=> $id,'std' => '','desc' => '','style' =>'','name' => 'Text Field');
            $new_field = array_merge($new_field, $args);
            if(false === $repeater){
                $this->_fields[] = $new_field;
            }else{
        ...
        public function addTextBoxs($id,$args,$repeater=false){
            $new_field = array('type' => 'textboxs','id'=> $id,'std' => '','desc' => '','style' =>'','name' => 'Text Field','fields'=>array('Name'));
            $new_field = array_merge($new_field, $args);
            if(false === $repeater){
                $this->_fields[] = $new_field;
            }else{
        ...
        public function addCustom($id,$args=array(),$repeater=false){
            $new_field = array('type' => 'custom','id'=> $id,'std' => '','desc' => '','style' =>'','name' => '');
            $new_field = array_merge($new_field, $args);
            if(false === $repeater){
                $this->_fields[] = $new_field;
            }else{
    ...
    <div class="apc_ie_panel field">        
        <div class="export_code">
          ...
          <textarea id="export_code"></textarea> <br />       
          <input class="button-primary" type="button" value="'. __('Export').'" id="apc_export_b" /> &nbsp; or &nbsp; '.$this->create_export_download_link().' &nbsp; <input class="button-primary" type="button"  value="'. __('Import').'" id="apc_import_b" />
          <div class="export_status" style="display: none;"><img width=50 src="'.$this->SelfPath.'/images/loading.gif" alt="loading..."/></div>
    ...
    ';
        ...
        public function export(){
            ...
            $options = get_option($_GET['group'],false);
        ...
        public function import(){
            ...
            if (!isset($_POST['imp']) || empty($_REQUEST['group'])){
            ...
            if (is_array($import_code)){
                update_option($_REQUEST['group'],$import_code);
        ...
        public function download_file($content = null, $file_name = null){
            ...
            $file_name = 'config_export_'.date('Y_m_d').'.txt';
        ...
        public function create_export_download_link($echo = false){
            ...
            if ($echo === true)
                echo '<a href="'.$export_url.'" target="_blank">Create Export File</a>';
            elseif ($echo == 'url')
                return $export_url;
            return '<a class="button-primary" href="'.$export_url.'" target="_blank">Create Export File</a>';
        }
        ...
        public function admin_redirect_download_files(){
            ...
            if (array_key_exists('theme_export_options', $_REQUEST) && $_REQUEST['theme_export_options'] == 'safe_download' ){
                $this->download_file();
                die();
            }
        }
        ...
        function Handle_plupload_action(){
            ...
            // handle file upload
            $status = wp_handle_upload($_FILES[$imgid . 'async-upload'], array('test_upload' => true, 'action' => 'plupload_action'));

            // send the uploaded file url in response

            $status = $this->move_upload_folder($status,$imgid,$has_field);
            if($status===false)return;
            echo $status['url'];
            exit;
        }

        function move_upload_folder($status,$imgid,$has_field)
        {
            $fileinfo = pathinfo($status['file']);
            $ext      = $fileinfo['extension'];
            $uploaddir = wp_upload_dir();

            $dir_upload = (empty($has_field['dir_upload']))? $fileinfo['dirname'] : $uploaddir['basedir'].'/'.$has_field['dir_upload'].'/';
            if(!is_dir($dir_upload))mkdir($dir_upload);

            //move upload dir custom
            if(!empty($has_field['dir_upload']))
            {
                if(!empty($status['file']))
                {
                    $filename = basename($status['file']);
                    $newfile = $dir_upload.$filename;
                    $newurl  = $uploaddir['baseurl'].'/'.$has_field['dir_upload'].'/'.$filename;
                    $rs = @rename($status['file'],$newfile);
                    if($rs)
                    {
                        $status['file'] = $newfile;
                        $status['url']  = $newurl;
            ...
            //check format
            if(!empty($has_field['format']))
            {
                $formats = explode(',',$has_field['format']);
                if(is_array($formats) && !in_array($ext,$formats))
                {
                    @unlink($status['file']);
                    echo 'error|Not allow file format: '.$ext;exit;
                }
            }
            //extract_zip
            if($has_field['extract_zip']==true && $ext =='zip')
            {
                @WP_Filesystem(array());
                $result = unzip_file($status['file'], $dir_upload);
                @unlink($status['file']);
            }
