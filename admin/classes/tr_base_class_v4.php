...
class Tr_Base_Class_V4{
    ...
    static function remove_role($name)
    {
    ...
    static function date($format='Y-m-d',$time=0)
    {
        if($time==0)
        {
            $time = time();
        ...
        return date($format, $time + $current_offset);
    }

    static function get_current_role()
    {
        $user = wp_get_current_user();
        $user_role = ($user && isset($user->roles[0]))? $user->roles[0] : '';
        if($user && empty($user_role) && isset($user->caps))
        {
            $user_role = array_keys($user->caps);
            $user_role = @$user_role[0];
        }
        return $user_role;
    }

    static function get_role($name)
    {
        global $wp_roles;
        if ( ! isset( $wp_roles ) )
            $wp_roles = new WP_Roles();

        return $wp_roles->get_role($name);
    }

    static function create_role($name,$display,$attributes = null,$default_role=null)
    {
        $result = get_role($name);
        if(!$result)
        {
    ...
    static public function getTemplatepage($templatefile,$return_url = false)
    {
        global $wpdb;
    ...
    static function content_nav( $nav_id ) {
        global $wp_query;
    ...
    static function get_Lat_LNG($my_address)
    {
        ...
        if(stripos($geocode,'Sorry...')!==false || $output ==null )
        {
            $request = new WP_Http;
    ...
    static function getaddress($lat,$lng,$key=null)
    {
